...
def main():
    ...
    restored_dir = f'/opt/swinir/images/restored/swinir_{args.task}_x{args.scale}_noise_{args.noise}_model_{model_name}'
    os.makedirs(restored_dir, exist_ok=True)
    log_file_path = os.path.join(restored_dir, f'celeba_sr_4_sy_{args.noise}_log.txt')
    logging.basicConfig(filename=log_file_path, level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    console_handler = logging.StreamHandler()
    console_handler.setLevel(logging.INFO)
    logging.getLogger().addHandler(console_handler)
    logging.info(f'{args.task} with Noise {args.noise} for CelebA Dataset using model {model_name}')
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        output_to_save = (output * 255.0).round().astype(np.uint8)  # float32 to uint8
        cv2.imwrite(f'{save_dir}/{imgname}_SwinIR.png', output_to_save)

        # evaluate psnr/ssim/psnr_b
        if img_gt is not None:
            # img_gt = (img_gt * 255.0).round().astype(np.uint8)  # float32 to uint8
            img_gt = img_gt[:h_old * args.scale, :w_old * args.scale, ...]  # crop gt
            img_gt = torch.from_numpy(np.squeeze(img_gt)).permute(2, 0, 1).unsqueeze(0)
            # print(img_gt.shape)
            # print(output.shape)
            # print(torch.from_numpy(output.transpose(2, 0, 1)).shape)
            # print(img_gt.get_device())
            # print(torch.from_numpy(output).get_device())
            mse = torch.mean((img_gt - torch.from_numpy(output.transpose(2, 0, 1)).unsqueeze(0)).to('cuda') ** 2)
            psnr = 10 * torch.log10(1 / mse)
            psnr_values.append(psnr.item())

            # Calculate LPIPS
            lpips_loss = lpips_loss_fn(img_gt.cuda(), torch.from_numpy(output.transpose(2, 0, 1)).unsqueeze(0).cuda())
            lpips_values.append(lpips_loss.item())

            # Log values for each iteration
            logging.info(
                f'index {idx}, image name {imgname}, PSNR: {psnr.item():.4f} dB, LPIPS: {lpips_loss.item():.4f}')
...
def setup(args):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        # save_dir = f'/opt/swinir/images/restored/swinir_{args.task}_x{args.scale}'
    ...
    # 004 grayscale image denoising/ 005 color image denoising
        ...
        # save_dir = f'/opt/swinir/images/restored/swinir_{args.task}_noise{args.noise}'
        # folder = args.folder_gt
        folder = args.folder_lq
        border = 0
        window_size = 8
    ...
    # 006 JPEG compression artifact reduction
        ...
        # save_dir = f'/opt/swinir/images/restored/swinir_{args.task}_jpeg{args.jpeg}'
        folder = args.folder_gt
        border = 0
        window_size = 7
...
def get_image_pair(args, path):
    ...
    img_idx = int(imgname[2:])
    # 001 classical image sr/ 002 lightweight image sr (load lq-gt image pairs)
    if args.task in ['classical_sr', 'lightweight_sr']:
        # img_gt = cv2.imread(path, cv2.IMREAD_COLOR).astype(np.float32) / 255.
        # img_lq = cv2.imread(f'{args.folder_lq}/{imgname}x{args.scale}{imgext}', cv2.IMREAD_COLOR).astype(
        #     np.float32) / 255.
        img_gt = cv2.imread(args.folder_gt + f"/orig_{img_idx}.png", cv2.IMREAD_COLOR).astype(np.float32) / 255.
